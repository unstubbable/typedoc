...
let TocPlugin = TocPlugin_1 = class TocPlugin extends components_1.RendererComponent {
    ...
    onRendererBeginPage(page) {
        ...
        TocPlugin_1.buildToc(model, trail, page.toc, tocRestriction, tocIncludeModules);
    }
    shouldBeIncludedInTrail(model) {
        if (model instanceof index_1.ProjectReflection) {
            return false;
        }
        return !model.kindOf(index_1.ReflectionKind.SomeModule) || this.owner.tocIncludeModules;
    }
    static buildToc(model, trail, parent, restriction, includeModules) {
        ...
        if (index < trail.length - 1 && children.length > 40) {
            ...
            children.forEach((child) => {
                ...
                if (child.kindOf(index_1.ReflectionKind.SomeModule) && !includeModules) {
                    return;
                }
                const item = NavigationItem_1.NavigationItem.create(child, parent, true);
